...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
	if (strcmp(name, "li") == 0) {
		/* YOUR CODE HERE */
		long int temp = strtol(args[1], NULL, 0);
		if (num_args != 2 || temp < -2147483648 || temp > 4294967295) {
			return 0;
		}
		if (-32768 <= temp && temp <= 65535) {
			fprintf(output, "%s %s %s %s\n", "addiu", args[0], "$0", args[1]);
			return 1;
		} else {
			fprintf(output, "%s %s %lu\n", "lui", "$at", temp >> 16);
			fprintf(output, "%s %s %s %lu\n", "ori", args[0], "$at", temp - (temp >> 16 << 16));
			return 2;
		}
...
int write_addiu(uint8_t opcode, FILE* output, char** args, size_t num_args) {
	...
	uint32_t instruction = (opcode << 26) + (rs << 21) + (rt << 16) + immi;
  	write_inst_hex(output, instruction);
  return 0;
}
